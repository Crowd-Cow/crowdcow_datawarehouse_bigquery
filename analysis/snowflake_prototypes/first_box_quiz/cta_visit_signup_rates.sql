...
,sign_up_from_customized_bundle as (
...
select
    ...
    ,count(distinct sign_up_event_fbq.visit_id)::float/count(distinct clicked_fbq_cta.visit_id) as "Pct Entered FBQ and Sign Up"
    ,count(distinct sign_up_from_bundle_select.visit_id)::float/count(distinct clicked_fbq_cta.visit_id)::float as "Pct Visits with Sign Up and Selected Bundle"
    ,count(distinct sign_up_from_customized_bundle.visit_id )::float/count(distinct clicked_fbq_cta.visit_id)::float as "Pct Visits with Sign Up and Selected Customized Bundle"
    ,count(distinct sign_up_from_custom_bundle.visit_id)::float/count(distinct clicked_fbq_cta.visit_id)::float as "Pct Visits with Sign Up and Selected Custom Bundle"
from clicked_fbq_cta
    left join sign_up_event_fbq on clicked_fbq_cta.visit_id = sign_up_event_fbq.visit_id
    left join sign_up_from_bundle_select on clicked_fbq_cta.visit_id = sign_up_from_bundle_select.visit_id
    left join sign_up_from_customized_bundle on clicked_fbq_cta.visit_id = sign_up_from_customized_bundle.visit_id
    left join sign_up_from_custom_bundle on clicked_fbq_cta.visit_id = sign_up_from_custom_bundle.visit_id
where
