...
select
    ...
    ,count(distinct clicked_nav_back.visit_id)::float/count(distinct clicked_fbq_cta.visit_id)::float as "Pct FBQ Backed Out"
    ,count(distinct clicked_on_bundle.visit_id)::float/count(distinct clicked_fbq_cta.visit_id)::float as "Pct FBQ Clicked Select Bundle"
    ,count(distinct clicked_on_custom.visit_id)::float/count(distinct clicked_fbq_cta.visit_id)::float as "Pct FBQ Clicked on Custom Box"
    ,count(distinct clicked_customize_bundle.visit_id)::float/count(distinct clicked_fbq_cta.visit_id)::float as "Pct FBQ Clicked Customize Bundle"
    ,count(distinct abandoned_visits.visit_id)::float/count(distinct clicked_fbq_cta.visit_id)::float as "Pct FBQ Abandond Session"
...
from clicked_fbq_cta
    ...
    left join abandoned_visits on clicked_fbq_cta.visit_id = abandoned_visits.visit_id
where
