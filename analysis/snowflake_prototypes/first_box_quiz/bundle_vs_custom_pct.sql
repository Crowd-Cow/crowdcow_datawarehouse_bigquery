...
select
    ...
    ,count(distinct clicked_purchase_bundle_order_complete.visit_id)::float/count(distinct fbq_flow.visit_id)::float as "Pct FBQ Purchased Bundle"
    ,count(distinct chose_custom_box_order_complete.visit_id)::float/count(distinct fbq_flow.visit_id)::float as "Pct FBQ Purchased Custom Box"
from fbq_flow
    left join clicked_purchase_bundle_order_complete on fbq_flow.visit_id = clicked_purchase_bundle_order_complete.visit_id
    left join chose_custom_box_order_complete on fbq_flow.visit_id = chose_custom_box_order_complete.visit_id
