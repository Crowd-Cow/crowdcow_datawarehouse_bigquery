...
, homepage_fbq_impressions as (
    select
        ...
        ,week(date(convert_timezone('UTC','America/Los_Angeles',fact_visit.visited_at))) as visit_week_num
        ,date_trunc(week, date(convert_timezone('UTC','America/Los_Angeles',fact_visit.visited_at)))::Date as visit_week
...
, aggregates as (
select
    homepage_fbq_impressions.visit_week
    ,experiment_variant
...
select aggregates.visit_week
,aggregates.experiment_variant
,coalesce(round(non_sub_paid_uncancelled_reorder_count::float / nullif(visitor_session_count,0), 3),0) as non_sub_paid_uncancelled_rate
from aggregates
