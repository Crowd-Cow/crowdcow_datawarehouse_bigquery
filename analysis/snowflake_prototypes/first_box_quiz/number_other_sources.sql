/**** Number of Visits to FBQ From Other Sources Than CTA ****/
...
other_fbq_traffic as (
    ...
    where name = 'page_view'
        and time >= '2021-08-25'
        and parse_json(properties):url::text like '%first-box%'
        and parse_json(properties):referrer_url::text like '%free-meat-promotion%'
)

select
    other_fbq_traffic.visit_date
    ,count(distinct other_fbq_traffic.visit_id) as fbq_cta_clicked_count
from other_fbq_traffic
group by 1
order by 1;
