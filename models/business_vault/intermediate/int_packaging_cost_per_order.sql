with packaging_costs as (select * from {{ ref('stg_gs__fc_care_packaging_costs') }} where cost_type = 'BOX_COST')
    ,box_types as (select * from  {{ ref( 'stg_cc__box_types' )}} )
    ,shipment_details as (select * from {{ ref( 'shipments') }} )
...
,get_box_id as (
    select month_to_cost_timing.month_of_costs
    ...
    from month_to_cost_timing
        join box_types on month_to_cost_timing.box_type = box_types.box_name
